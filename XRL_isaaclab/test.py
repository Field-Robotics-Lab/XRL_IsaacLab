...
class XrlIsaaclabEnv(DirectRLEnv):
    ...
    def _setup_scene(self):
        ...
        self.vel_commands = torch.randn((self.cfg.scene.num_envs, 3)).cuda()+5 #differentiate between velocity commands and position commands
    ...
    def _visualize_markers(self):
        ...
        self.visualization_markers.visualize(loc, self.forward_marker_orientations, marker_indices=indices)
    ...
    def _get_observations(self) -> dict:
        ...
        obs = torch.hstack((forward_speed, dist))

        observations = {"policy": obs}
        return observations

    def _get_rewards(self) -> torch.Tensor:
        forward_reward = torch.abs(self.robot.data.root_com_lin_vel_b[:,0].reshape(-1,1))
        alignment_reward = torch.sum(self.forwards * self.pose_commands, dim=-1, keepdim=True)
        ...
        d_0 = 1
        distance_reward = 1-(d_0/dist) #minimize the distance from agent to target
        total_reward = forward_reward + distance_reward
        #print(forward_reward, distance_reward)
        return total_reward
